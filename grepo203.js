                ...
                if (build_object.hasFreeBuildingSlots(_0x101284)) {
                    ...
                    if (void 0 !== building_item) {
                        ...
                        _0x936f75.can_upgrade ? api_object.frontend_bridge(build_object.town.id, {
                            ...
                            response.error && console_object.Log(build_object.town.name + " " + response.error, 3), build_object.startQueueing();
                        }) : _0x936f75.enough_population ? _0x936f75.enough_resources ? (console_object.Log(build_object.town.name + " " + building_item.item_name + " can not be started due dependencies.", 3), 
                        api_object.Auth("removeItemQueue", {
                            player_id: autobot_object.Account.player_id,
                            world_id: autobot_object.Account.world_id,
                            csrfToken: autobot_object.Account.csrfToken,
                        ...
                        $(".queue_id_" + building_item.id).remove(), build_object.finished()) : (console_object.Log(build_object.town.name + " not enough resources for " + building_item.item_name + ".", 3), build_object.finished()) : (console_object.Log(build_object.town.name + " not enough population for " + building_item.item_name + ".", 3), build_object.finished());
                    } else build_object.finished2();
                } else console_object.Log(build_object.town.name + " no free building slots available.", 3), build_object.finished2();
            });
        }, autobot_object.randomize(1e3, 2e3)) : build_object.finished2();
    },
    finished2 : function() {},
