...
var farm_object = {
    ...
    initFarmTownsCaptain: function(callback) {
        /** Rellena la lista shouldFarm con las granjas de la ciudad. */
    ...
    claimResources: function() {
        ...
        farm_object.interval = setTimeout(function() {
            var type = "normal";
            if (Game.features.battlepoint_villages || (farm_object.shouldFarm[farm_object.town.currentFarmCount].mood >= 86 && farm_object.settings.stoplootbelow && (type = "double"), farm_object.settings.stoplootbelow || (type = "double")), farm_object.isCaptain) {
                var farm_list = [];
            ...
            } else console_object.Log(farm_object.shouldFarm[farm_object.town.currentFarmCount].id + " started.", 1), farm_object.claimLoad(farm_object.shouldFarm[farm_object.town.currentFarmCount].id, type, function() {
                if (!farm_object.checkEnabled()) return false;
        ...
        autobot_object.randomize(1000 * farm_object.settings.timebetween, 1000 * farm_object.settings.timebetween + 1000));
            ...
            if(farm_object.shouldFarm.length > 0){ 
                console_object.Log(" hABÍAS PARA FARMEAR.", 1);
                    $.each(farm_object.shouldFarm, function(index, farm) {
                        var diff_time = farm.loot - Timestamp.now();
                        (null == time || diff_time <= time) && (time = diff_time);
                    }); 
                }
            else  
                    ...
                    if (void 0 !== building_item) {
                        ...
                        _0x936f75.can_upgrade ? api_object.frontend_bridge(build_object.town.id, {
                            ...
                            if (response.success) {
                                ...
                                console_object.Log('<span style="color: #ffa03d;">' + building_item.item_name.capitalize() + " - " + response.success + "</span>", 3), api_object.Auth("removeItemQueue", {
                                    player_id: autobot_object.Account.player_id,
                                    world_id: autobot_object.Account.world_id,
                                    csrfToken: autobot_object.Account.csrfToken,
                                    town_id: build_object.town.id,
                                    ...
                                    type: "building"
                                }, build_object.callbackSaveBuilding($("#building_tasks_main .ui_various_orders, .construction_queue_order_container .ui_various_orders"))), $(".queue_id_" + building_item.id).remove();
                            }
                            response.error && console_object.Log(build_object.town.name + " " + response.error, 3), build_object.finished();
                        }) : _0x936f75.enough_population ? _0x936f75.enough_resources ? (console_object.Log(build_object.town.name + " " + building_item.item_name + " can not be started due dependencies.", 3), api_object.Auth("removeItemQueue", {
                            player_id: autobot_object.Account.player_id,
                            world_id: autobot_object.Account.world_id,
                            csrfToken: autobot_object.Account.csrfToken,
                            town_id: build_object.town.id,
                            item_id: building_item.id,
                            type: "building"
                        }, build_object.callbackSaveBuilding($("#building_tasks_main .ui_various_orders, .construction_queue_order_container .ui_various_orders"))), $(".queue_id_" + building_item.id).remove(), build_object.finished()) : (console_object.Log(build_object.town.name + " not enough resources for " + building_item.item_name + ".", 3), build_object.finished()) : (console_object.Log(build_object.town.name + " not enough population for " + building_item.item_name + ".", 3), build_object.finished());
                    } else build_object.finished();
            ...
            var data = {
                ...
                table_content = $("<table/>", {
                    ...
                    return $.each(data, function(key, value) {
                        ...
                        style: "background-color: #DFCCA6;width: 30%;"
                    }).html("Premium:")).append($("<td/>").append(autobot_object.premium_time - Timestamp.now() >= 0 || true ? autobot_object.secondsToTime(autobot_object.premium_time - Timestamp.now()) : "No premium").append($("<div/>", {
                        id: "premium-bot"
                    }).append($("<div/>", {
                        class: "js-caption"
