...
`));
    ...
    var main_object = {
        ...
        checkWhatToStart: function() {
            var i = 0;
            $.each(main_object.modules, function(index, module) {
                module.isOn && "Autoattack" !== module && i++;
            }), 0 === i ? main_object.stop() : i >= 0 && !main_object.Queue.isRunning() && (clearInterval(main_object.interval), main_object.start());
        },
        ...
        callbackAuth: function(response) {
            ...
            autobot_object.trial_time - Timestamp.now() >= 0 || true || autobot_object.premium_time - Timestamp.now() >= 0 ? (main_object.hasPremium = true, main_object.init(), farm_object.init(), farm_object.setSettings(response.autofarm_settings), culture_object.init(), culture_object.setSettings(response.autoculture_settings), build_object.init(), build_object.setSettings(response.autobuild_settings), build_object.setQueue(response.building_queue, response.units_queue, response.ships_queue), attack_object.init(), main_object.checkAutostart()) : (main_object.hasPremium = false, main_object.init(), farm_object.init(), $("#Autoculture_onoff").mousePopup(new MousePopup(main_object.requiredPrem)), $("#Autobuild_onoff").mousePopup(new MousePopup(main_object.requiredPrem)), $("#Autoattack_onoff").mousePopup(new MousePopup(main_object.requiredPrem)), autobot_object.createNotification("getPremiumNotification", "Unfortunately your premium membership is over. Please upgrade now!"));
        },
        models: {
            Town: function() {
