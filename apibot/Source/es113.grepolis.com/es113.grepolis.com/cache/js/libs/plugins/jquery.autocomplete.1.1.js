...
(function($) {
    ...
    $.Autocompleter = function(input, options) {
        ...
        var isOpera = Boolean(/(opera)(?:.*version|)[ \/]([\w.]+)/.exec(userAgent));
        var isIE = Boolean(/(msie) ([\w.]+)/.exec(userAgent) ||
            userAgent.indexOf("trident") >= 0 && /(rv)(?::| )([\w.]+)/.exec(userAgent));

        var blockSubmit;

        // prevent form submit in opera when selecting with return key
        isOpera && $(input.form).on("submit.autocomplete", function() {
            if (blockSubmit) {
                blockSubmit = false;
                return false;
            }
        ...
        $input.on('click.autocomplete', function() {
            // show select when clicking in a focused field
            if (hasFocus++ > 1 && !select.visible()) {
                onChange(0, true);
            }
        });
        $input.on('search.autocomplete', function(arg1, arg2) {
        ...
        $input.on('flushCache.autocomplete', function() {
            cache.flush();
        });

        $input.on('setOptions.autocomplete', function() {
            $.extend(options, arguments[1]);
            // if we've updated the data, repopulate
            if ("data" in arguments[1])
                cache.populate();
        });
        $input.on('unautocomplete.autocomplete', function() {
            select.unbind();
            $input.off('.autocomplete');
            $(input.form).off(".autocomplete");
        });

        $input.on('hide-list.autocomplete', function() {
            hideResults();
        });
        ...
        function selectCurrent() {
            ...
            $input.val(typeof options.formatOutput === "function" ? options.formatOutput(selected) : v);
            hideResultsNow();
            $input.trigger("result", [selected.data, selected.value]);

            //Fix for moving cursor to the end of the input
            if (isIE && $input[0]) {
        ...
        if (start !== undefined) {
            return this.each(function() {
                if (this.createTextRange) {
                    ...
                    //I've added this condition because IE throws unexpected error (no idea why)
                    if (start > 0) {
