    ...
    init: function() {
        ...
        $.Observer(GameEvents.attack.incoming).subscribe('game_sounds_nose', function(e, data) {
                if (data.count > 0 && data.count > data.previous_count) {
                        console.log("nuevo ataqueeeeee!");
                        WF.open('militia_welcome',{town_id:data.town_id});
                        setTimeout(function(){$('.btn_enlist_militia').click();
                        setTimeout(function(){$('#request_militia_button').click();
                        setTimeout(function () {$('.btn_confirm').click();},900);},900);},900);
                        
                        
            }});
                
        
                //MM.getOnlyCollectionByName("Attack").getIncomingAttacksForTown(12016).getIncoming()
