...
var farm_object = {
    checkReady: function(town) {
        ...
        var town_resources = current_town.resources();
        if (town_resources.wood === town_resources.storage && town_resources.stone === town_resources.storage && town_resources.iron === town_resources.storage && farm_object.settings.skipwhenfull) return false;
        var _0xad9770 = false;
        return $.each(main_object.Queue.queue, function(index, queue_element) {
            if ("Autofarm" === queue_element.module && -1 !== town.relatedTowns.indexOf(queue_element.townId)) return _0xad9770 = true, false;
            ...
            var _0x37562f = current_town.resources(),
    ...
    initFarmTowns: function(callback) {
        api_object.game_data(farm_object.town.id, function(response) {
            if (!farm_object.checkEnabled()) return false;
            var _0x364e4d = response.map.data.data.data;
            $.each(_0x364e4d, function(index, _0x3e7771) {
                var _0x3168fd = [];
                $.each(_0x3e7771.towns, function(index, _0x4c62b9) {
...
var main_object = {
    ...
    start: function() {
        ...
        if ($.each(main_object.playerTowns, function(index, town) {
                ...
                if(ready){  
                        (flag = true, main_object.Queue.add({
                                                                townId: town.id,
                                                                fx: function() {
                                                                    town.startFarming();
                                                                }
                ...
                var culture_ready = culture_object.checkReady(town);
                if(culture_ready) {
                    (flag = true, main_object.Queue.add({
                    townId: town.id,
                    fx: function() {
                ...
                }))} else { false !== culture_ready && (null == _0x2ca9c8 || culture_ready < _0x2ca9c8) && (_0x2ca9c8 = culture_ready);
                }

                var build_ready = build_object.checkReady(town);
                if(build_ready) {(flag = true, main_object.Queue.add({
                    townId: town.id,
                    fx: function() {
                        town.startBuild();
                    }
                })) } else {
                     false !== build_ready && (null == _0x2ca9c8 || build_ready < _0x2ca9c8) && (_0x2ca9c8 = build_ready);
                }
            }), null !== _0x2ca9c8 || flag)
