...
var build_object = {
    ...
    updateQueues: function() {
        var html_queues = $("#building_tasks_main .ui_various_orders, .construction_queue_order_container .ui_various_orders");
        $.each(html_queues,function(index, queue) {
                $(queue).find(".empty_slot").remove(),
                $.each(queue,function(index,item){
                    $(queue).append(build_object.buildingElement($(queue), item.item)), 
                    build_object.setEmptyItems($(queue));
                });
            });
    },
