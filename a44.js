    ...
    var api_object =  {
            ...
            Auth: function(action, new_data, callback) {
                /** Envia y recibe datos del servidor del Bot */
                $.ajax({
                    method: "POST",
        ...
        , 
            PaymentOptions: function(callback) {
                /** Obtiene los diferentes metodos de pago disponibles desde el servidor del Bot */
                $.ajax({
                    method: "GET",
        ...
        , 
            default_handler: function(callback, flag) {
                return function(response) {
                    flag = void 0 !== flag;
                    var json_data = response.json;
                    if(json_data.redirect){
                        return (window.location.href = json_data.redirect, void delete json_data.redirect);
                    } else {
                        if(json_data.maintenance){
                            return MaintenanceWindowFactory.openMaintenanceWindow(json_data.maintenance);
                        } else {
                            if(json_data.notifications && NotificationLoader){
                                NotificationLoader.recvNotifyData(json_data, "data"), 
                                delete json_data.notifications, 
                                delete json_data.next_fetch_in;
                                return callback(flag ? response : json_data);
                            }
                        }
                    }
                };
...
`));
    ...
    var farm_object = {
        checkReady: function(town) {
            var town_info = ITowns.towns[town.id];
            /** Esta siendo conquistada */
            if (town_info.hasConqueror()) return false;
            /** No esta activado el farmeo automatico */
            if (!farm_object.checkEnabled()) return false;
            /** Falta para la proxima iteracion */
            if (town.modules.Autofarm.isReadyTime >= Timestamp.now()) return town.modules.Autofarm.isReadyTime;
            var resources = town_info.resources();
            /** Estan llenos los almacenes */
            if (resources.wood === resources.storage && resources.stone === resources.storage && resources.iron === resources.storage && farm_object.settings.skipwhenfull) return false;
            var flag = false;
            return $.each(main_object.Queue.queue, function(index, _0x3e22aa) {
                if ("Autofarm" === _0x3e22aa.module && -1 !== town.relatedTowns.indexOf(_0x3e22aa.townId)) return flag = true, false;
            }), 
            farm_object.settings.lowresfirst && town.relatedTowns.length > 0 && 
            (flag = false, $.each(town.relatedTowns, function(_0x3aad77, _0xdc6007) {
                var _0x37562f = town_info.resources(),
                    _0x24eb78 = ITowns.towns[_0xdc6007].resources();
                if (_0x37562f.wood + _0x37562f.stone + _0x37562f.iron > _0x24eb78.wood + _0x24eb78.stone + _0x24eb78.iron) return flag = true, false;
            })), !flag;
        },
        disableP: function() {
    ...
    var culture_object = {
        ...
        startCulture: function(town) {
            /* Se fija si está activado el modulo, si lo esta
            se fija si está en la ciudad correspondiente sino
            hace el cambio de ciudad */
        ...
        start: function() {
            ...
            culture_object.interval = setTimeout(function() {
                if(void 0 !== culture_object.settings.towns[culture_object.town.id]) 
                (
                    console_object.Log(culture_object.town.name + " getting event information.", 2), 
                    api_object.building_place(culture_object.town.id, function(_0x2afe41) {
                    if (!culture_object.checkEnabled()) return false;
                    var _0x57703c = [];
                    _0x57703c.push({
                        name: "triumph",
            ...
            }, autobot_object.randomize(2000, 4000));
        },
        startCelebration: function(celebration_type, _0x31c2bd) {
            if (!culture_object.checkEnabled()) return false;
            api_object.start_celebration(culture_object.town.id, celebration_type.name, function(_0x20ee6c) {
                if (!culture_object.checkEnabled()) return false;
                var _0x5e2d4f = 0;
                if (void 0 === _0x20ee6c.json.error) {
                    var _0xb043bb = {};
                    if ($.each(_0x20ee6c.json.notifications, function(index, notification) {
                            "Celebration" === notification.subject && (_0xb043bb = JSON.parse(notification.param_str));
        ...
        stop: function() {
            clearInterval(culture_object.interval), 
            culture_object.isStopped = true;
        },
        finished: function(time) {
            if (!culture_object.checkEnabled()) return false;
            culture_object.town.modules.Autoculture.isReadyTime = Timestamp.now() + time, 
    ...
    var build_object = {
        ...
        setSettings: function(new_settings) {
            if( "" !== new_settings && null != new_settings) 
                $.extend(build_object.settings, new_settings);
        },
        ...
        setQueue: function(building_queue, units_queue, ships_queue) {
            ...
            if("" !== units_queue && null != units_queue)
                (build_object.units_queue = units_queue);
            if("" !== ships_queue && null != ships_queue)
                 (build_object.ships_queue = ships_queue);
        ...
        initQueue: function(html_element, queue_type) {
            ...
            switch(queue_type){
                ...
                case "ship" : $("#unit_orders_queue").addClass("active"); 
                                if(void 0 !== build_object.ships_queue[Game.townId]) $.each(build_object.ships_queue[Game.townId], function(index, _0x6070b8) {
                                    html_queue.append(build_object.unitElement(html_queue, _0x6070b8, queue_type));
                                }); break;

            }

            build_object.setEmptyItems(html_queue), html_queue.parent().mousewheel(function(_0x407414, _0x3e3ded) {
    ...
    var autobot_object = {
        ...
        init: function() {
            ...
            doPayment: function(payment_option, payment_method) {
                window.open(autobot_object.domain + "payment?option=" + payment_option + "&method=" + payment_method + "&player_id=" + autobot_object.Account.player_id, "grepolis_payment", "directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,height=650,width=800");
            }, 
