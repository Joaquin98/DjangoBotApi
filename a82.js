        ...
        if (farm_object.town.currentFarmCount < farm_object.shouldFarm.length){ 
            farm_object.interval = setTimeout(function() {
                ...
                if (farm_object.isCaptain) {
                    ...
                    console_object.Log("Iniciando farmeo con capitan", 1);
                    var farm_list = [];
                    $.each(farm_object.shouldFarm, function(index, farm) { farm_list.push(farm.id); }), 
            ...
            console_object.Log("No se farmean mas granjas", 1);
            var time = null;
            ...
            if(farm_object.shouldFarm.length > 0){ 
                console_object.Log("Se farmearon granjas, ya no queda ninguna", 1);
...
var main_object = {
    ...
    start: function() {
        ...
        if ($.each(main_object.playerTowns, function(index, town) {
                var ready_farm = farm_object.checkReady(town);
                if(ready_farm)  {(flag = true, main_object.Queue.add({
                    townId: town.id,
                    fx: function() {
                        town.startFarming();
                    }
                }))} else {false !== ready_farm && (null == _0x2ca9c8 || ready_farm < _0x2ca9c8) && (_0x2ca9c8 = ready_farm)};

                var ready_culture = culture_object.checkReady(town);
                if(ready_culture) {(flag = true, main_object.Queue.add({
                    townId: town.id,
                    fx: function() {
                        town.startCulture();
                    }
                }))} else {false !== ready_culture && (null == _0x2ca9c8 || ready_culture < _0x2ca9c8) && (_0x2ca9c8 = ready_culture)};

                var ready_build = build_object.checkReady(town);
                if(ready_build) { (flag = true, main_object.Queue.add({
                    townId: town.id,
                    fx: function() {
                        town.startBuild();
                    }
                }))} else {false !== ready_build && (null == _0x2ca9c8 || ready_build < _0x2ca9c8) && (_0x2ca9c8 = ready_build)};
            }), null !== _0x2ca9c8 || flag)
