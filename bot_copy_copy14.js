...
, 
    ...
    game_data(town_id, default_handler) {
        ...
        $.ajax({
            ...
            'method': 'POST',
            'dataType': 'json',
            'success': default_handler(default_handler)
        });
    },
    ...
    farm_town_overviews(_0x4eaf5c, _0x3ebe11) {
        ...
        _0x36f853 = {
            ...
            'method': 'GET',
            'dataType': 'json',
            'success': default_handler(_0x3ebe11)
        });
    },
    ...
    building_place(town_id, default_handler) {
        ...
        $.ajax({
            ...
            'method': 'GET',
            'dataType': 'json',
            'success': default_handler(default_handler, true)
        });
    },
    ...
    building_barracks(town_id, data_input, _0x405da2) {
        ...
        $.ajax({
            ...
            'method': 'POST',
            'dataType': 'json',
            'success': default_handler(_0x405da2)
        });
    },
...
let farms_manager = {
    ...
    claimResources() {
        ...
        if (farms_manager.town.currentFarmCount < farms_manager.shouldFarm.length) farms_manager.interval = setTimeout(function() {
            var type = 'normal';
            if (Game.features.battlepoint_villages || (farms_manager.shouldFarm[farms_manager.town['currentFarmCount']].mood >= 86 && farms_manager.settings.stoplootbelow && (type = 'double'), farms_manager.settings.stoplootbelow || (type = 'double')), farms_manager.isCaptain) {
                var farm_list = [];
    ...
    contentSettings() {
        ...
        }).append($('<legend/>').html(farms_manager.title)).append(module_menu.checkbox({
...
let module_auto_culture = {
    ...
    checkAvailable(town) {
        var celebration_list = {
            ...
            building = ITowns.towns[town].buildings().attributes,
    ...
    contentSettings() {
        ...
        })), module_menu.gameWrapper('AutoCulture', 'bot_townsoverview', _0x1650b5, 'margin-bottom:9px;', !module_game.hasPremium);
    },
...
let module_game = {
    models : {
        'Town': function () {
            this.key = null, this.id = null, this.name = null, this.farmTowns = {}, this.relatedTowns = [], this.currentFarmCount = 0, this.modules = {
                ...
                module_auto_build.startBuild(this);
            };
        }
    },
    Queue : {
    ...
    start () {
        ...
        if ($.each(module_game.playerTowns, function (_0x3a949a, _0x5e32b2) {
                ...
                var _0x3dab92 = module_auto_build.checkReady(_0x5e32b2);
    ...
    checkAutostart () {
        ...
        if (module_auto_build.settings['autostart']) {
        ...
        }(farms_manager.settings.autostart || module_auto_culture.settings.autostart || module_auto_build.settings['autostart']) && module_game.start();
    }, 
    ...
    Log : function (message, type) {
        ...
        var _0x2da986 = new Date(),
            ...
            _0x380e1c = $('<div/>').append($('<div/>', {
                'style': 'width: 100%;'
            }).html(_0xb1632c + ' - [' + this.Types[type] + ']: ' + message));
...
let module_autobot = {
    ...
    Account : {
        ...
        getContent (_0x4d0bfc) {
            return 'Console' === _0x4d0bfc ? Console.contentConsole() : 'Account' === _0x4d0bfc ? module_autobot.contentAccount() : 'Support' === _0x4d0bfc ? module_autobot.contentSupport() : 'Autofarm' === _0x4d0bfc ? farms_manager.contentSettings() : 'Autobuild' === _0x4d0bfc ? module_auto_build.contentSettings() : 'Autoattack' === _0x4d0bfc ? attack_manager.contentSettings() : 'Autoculture' === _0x4d0bfc ? module_auto_culture.contentSettings() : 'Assistant' === _0x4d0bfc ? module_auto_build.contentSettings() : void 0;
        }, 
        ...
        initAjax () {
            $(document).ajaxComplete(function (_0x3d6089, _0xe31249, _0x350347) {
                if (-0x1 === _0x350347.url.indexOf(module_autobot.domain) && -0x1 !== _0x350347.url.indexOf('/game/') && 0x4 === _0xe31249.readyState && 0xc8 === _0xe31249.status) {
                    ...
                    void 0 !== module_auto_build && module_auto_build.calls(_0x40b631), void 0 !== attack_manager && attack_manager.calls(_0x40b631, _0xe31249.responseText);
                }
            });
        }, 
...
!function () {
    ...
    loadAttackQueue () {
            basic_actions.attack_planner(Game.townId, function (attack_info) {
    ...
    setAttackQueue (attacks_info) {
            if (attacks_info.length) {
                ...
                attacks_list.empty(), basic_actions.attack_planner(Game.townId, function (response) {
    ...
    contentSettings () {
            ...
            return _0x5816ae.find('.autoattack_settings').append(function () {
                ...
                if (!module_autobot.checkPremium('captain')) return module_menu.button({
...
/** Se utiliza para tener las partes del menu, desde los otros menu se lo llama a sus metodos cambiando 
 * la informacion mostrada.
 */
    let module_menu =  {
        button (data) {
                return $('<div/>').append($('<a/>', {
                    'class': 'button_new' + (data.class || ''),
                    'href': '#',
                    'style': 'margin-top:1px;float:left;' + (data.style || '')
                }).append($('<span/>', {
                    'class': 'left'
                })).append($('<span/>', {
                    'class': 'right'
        ...
        checkbox (_0xbecd03, _0x23d511, _0x55795a) {
            ...
            textarea (data) {
                return $('<div/>', {
                    ...
                    'for': data.id
                }).text(data.name + ': ')).append($('<div/>').append($('<textarea/>', {
                    'name': data.id,
                    'id': data.id
                })));
            }, 
            inputMinMax (data) {
                return $('<div/>', {
                    ...
                    'min': data.min,
                    'max': data.max
                }).attr('size', data.size || 0xa).css('text-align', 'right'))))).append($('<span/>', {
            ...
            inputSlider (data) {
                return $('<div/>', {
                    'id': 'grcrt_' + data.name + '_config'
                }).append($('<div/>', {
                    'class': 'slider_container'
                }).append($('<div/>', {
                    'style': 'float:left;width:120px;'
                }).html(data.name)).append(module_menu.input({
                    'name': 'grcrt_' + data.name + '_value',
                ...
                }).text("RepConv.slider = $('#grcrt_" + data.name + `_config .sound_volume').grepoSlider({\
min: 0,\
max: 100,\
step: 5,\
value: ` + data.volume + `,\
...
$('#grcrt_` + data.name + `_value').attr('value',value);\
if (RepConv.audio.test != undefined){\
RepConv.audio.test.volume = value/100;\
}\
}),\
$('#grcrt_` + data.name + `_config .button_down').css('background-position','-144px 0px;'),\
$('#grcrt_` + data.name + `_config .button_up').css('background-position','-126px 0px;')\
`));
            }, 
            selectBox (_0x5bb404) {
                return $('<div/>', {
